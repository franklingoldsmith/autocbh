...
class TestMol2Graph:
    ...
    normal_smile = 'CC(=O)CC'

    adsorbate_single_bond = 'CC(=O)C[Pt]'
    adsorbate_double_bond = 'CC(=O)C=[Pt]'
    adsorbate_triple_bond = 'CC(=O)C#[Pt]'

    radical_singlet = 'CC(=O)C[CH2]'
    radical_doublet = 'CC(=O)C[CH]'
    radical_triplet = 'CC(=O)C[C]'
...
class TestCBH0Ffunc:
    ...
    smiles3 = 'C(F)(F)(F)C(F)(F)F' # no H's

    def test_generic_pdts(self):
        smiles1 = 'CC(=O)C(F)(F)C' # C O F H
        cbh = CBH.buildCBH(smiles1)
        pdts, rcts = cbh.CBH_0_F()
        assert pdts == {'C':3.5, 'O':1, 'FC(F)(F)F':0.5}
    ...
    def test_no_oxy_pdts(self):
        ...
        pdts, rcts = cbh.CBH_0_F()
        assert pdts == {'C':3.5, 'FC(F)(F)F':0.5}
    ...
    def test_no_h_or_o_pdts(self):
        ...
        pdts, rcts = cbh.CBH_0_F()
        assert pdts == {'C':0.5, 'FC(F)(F)F':1.5}
    ...
    def test_no_h_pdts(self):
        ...
        pdts, rcts = cbh.CBH_0_F()
        assert pdts == {'C':1, 'O':1, 'FC(F)(F)F':1}
...
def test_count_repeats():
    ls = ['CC(F)C', 'C', 'C', 'FCF', 'FCF', 'FCF']
    repeats = CBH.buildCBH.count_repeats(ls)
    assert repeats == {'CC(F)C':1, 'C':2, 'FCF':3}
...
class TestAtomCentric:
    ...
    def test_no_atom_inds(self):
        ...
        correct = ['C[Pt]', 'C(=O)(N)[Pt]', 'C=O', 'CNS', 'NSF', 'SF']
        correct = [Chem.CanonSmiles(s) for s in correct]
        assert set(correct) == residuals
    ...
    def test_no_atom_inds_sat(self):
        ...
        correct = ['C(F)(F)(F)[Pt]', 'C(=O)(N)[Pt]', 'C(F)(F)=O', 'C(F)(F)(F)NS', 'NSF', 'N']
        correct = [Chem.CanonSmiles(s) for s in correct]
        assert set(correct) == residuals
    ...
    def test_sat_10(self):
        ...
        correct = ['ClC(=O)NSF', 'ClC(=O)NSF', 'ClC(=O)NSF', 'ClC(=O)NSF', 'ClC(=O)NSF', 'ClC(=O)NSF']
        correct = [Chem.CanonSmiles(s) for s in correct]
        assert set(correct) == residuals
    ...
    def test_sat_10_noH(self):
        ...
        correct = ['ClC(=O)N(F)SF', 'ClC(=O)N(F)SF', 'ClC(=O)N(F)SF', 'ClC(=O)N(F)SF', 'ClC(=O)N(F)SF']
        correct = [Chem.CanonSmiles(s) for s in correct]
        assert set(correct) == residuals
...
class TestBondCentric:
    ...
    def test_saturation(self):
        ...
        correct = ['C(F)(F)(F)Cl', 'C(F)(F)=O', 'C(F)(F)(F)N', 'SN', 'N']
        correct = [Chem.CanonSmiles(s) for s in correct]
        assert set(correct) == residuals
    ...
    def test_dist_1(self):
        ...
        correct = ['C(Cl)(=O)N', 'C(Cl)(=O)NS', 'CNSF', 'NSF']
        correct = [Chem.CanonSmiles(s) for s in correct]
        assert set(correct) == residuals
    ...
    def test_dist_1_sat(self):
        ...
        correct = ['C(Cl)(=O)N', 'C(Cl)(=O)NS', 'C(F)(F)(F)NSF', 'C(F)(F)(F)NS']
        correct = [Chem.CanonSmiles(s) for s in correct]
        assert set(correct) == residuals
...
class TestBuildSchemeGeneral:
    ...
    def test_general_cbh_rcts_r0(self):
        smiles = 'ClC(=O)NSF'
        cbh = CBH.buildCBH(smiles)
        assert cbh.cbh_rcts[0] == {'[H][H]': 6.0}

    def test_general_cbh_rcts_r1(self):
        smiles = 'ClC(=O)NSF'
        cbh = CBH.buildCBH(smiles)
        assert cbh.cbh_rcts[1] == {'N': 1, 'S': 1, 'C': 2}
    
    def test_general_cbh_rcts_r2(self):
        smiles = 'ClC(=O)NSF'
        cbh = CBH.buildCBH(smiles)
        assert cbh.cbh_rcts[2] == {'CN': 1, 'NS': 1}

    def test_general_cbh_rcts_r3(self):
        smiles = 'ClC(=O)NSF'
        cbh = CBH.buildCBH(smiles)
        assert cbh.cbh_rcts[3] == {'CNS': 1}
    
    def test_general_cbh_pdts_r0(self):
        smiles = 'ClC(=O)NSF'
        cbh = CBH.buildCBH(smiles)
        assert cbh.cbh_pdts[0] == {'O': 1, 'F': 1, 'Cl': 1, 'N': 1, 'S': 1, 'C': 1}

    def test_general_cbh_pdts_r1(self):
        smiles = 'ClC(=O)NSF'
        cbh = CBH.buildCBH(smiles)
        assert cbh.cbh_pdts[1] == {'CN': 1, 'CCl': 1, 'NS': 1, 'FS': 1, 'C=O': 1}

    def test_general_cbh_pdts_r2(self):
        smiles = 'ClC(=O)NSF'
        cbh = CBH.buildCBH(smiles)
        assert cbh.cbh_pdts[2] == {'NSF': 1, 'NC(=O)Cl': 1, 'CNS': 1}

    def test_general_cbh_pdts_r3(self):
        smiles = 'ClC(=O)NSF'
        cbh = CBH.buildCBH(smiles)
        assert cbh.cbh_pdts[3] == {'O=C(Cl)NS': 1, 'CNSF': 1}
...
class TestBuildSchemeOvershoot:
    ...
    def test_overshoot_highest_rung(self):
        ...
        assert cbh.highest_cbh == 1
    
    def test_overshoot_cbh_rcts_r0(self):
        smiles = 'CC(F)(F)Cl'
        cbh = CBH.buildCBH(smiles, allow_overshoot=True)
        assert cbh.cbh_rcts[0] == {'[H][H]': 4.0}

    def test_overshoot_cbh_rcts_r1(self):
        smiles = 'CC(F)(F)Cl'
        cbh = CBH.buildCBH(smiles, allow_overshoot=True)
        assert cbh.cbh_rcts[1] == {'C': 3}
    
    def test_overshoot_cbh_pdts_r0(self):
        smiles = 'CC(F)(F)Cl'
        cbh = CBH.buildCBH(smiles, allow_overshoot=True)
        assert cbh.cbh_pdts[0] == {'Cl': 1, 'F': 2, 'C': 2}

    def test_overshoot_cbh_pdts_r1(self):
        smiles = 'CC(F)(F)Cl'
        cbh = CBH.buildCBH(smiles, allow_overshoot=True)
        assert cbh.cbh_pdts[1] == {'CCl': 1, 'CC': 1, 'CF': 2}
    ...
    def test_overshoot_highest_rung_sat(self):
        ...
        assert cbh.highest_cbh == 2
    
    def test_overshoot_cbh_rcts_r0_sat(self):
        smiles = 'CC(F)(F)Cl'
        cbh = CBH.buildCBH(smiles, 9, allow_overshoot=True)
        assert cbh.cbh_rcts[0] == {'[H][H]': 0.5, 'FF': 4.5}

    def test_overshoot_cbh_rcts_r1_sat(self):
        smiles = 'CC(F)(F)Cl'
        cbh = CBH.buildCBH(smiles, 9, allow_overshoot=True)
        assert cbh.cbh_rcts[1] == {'FC(F)(F)F': 4}
    
    def test_overshoot_cbh_rcts_r2_sat(self):
        smiles = 'CC(F)(F)Cl'
        cbh = CBH.buildCBH(smiles, 9, allow_overshoot=True)
        assert cbh.cbh_rcts[2] == {'FC(F)(F)C(F)(F)F': 1}

    def test_overshoot_cbh_pdts_r0_sat(self):
        smiles = 'CC(F)(F)Cl'
        cbh = CBH.buildCBH(smiles, 9, allow_overshoot=True)
        assert cbh.cbh_pdts[0] == {'Cl': 1, 'F': 3, 'FC(F)(F)F': 2}

    def test_overshoot_cbh_pdts_r1_sat(self):
        smiles = 'CC(F)(F)Cl'
        cbh = CBH.buildCBH(smiles, 9, allow_overshoot=True)
        assert cbh.cbh_pdts[1] == {'FC(F)(F)C(F)(F)F': 1, 'FC(F)(F)Cl': 1, 'FC(F)F': 3}
...
class TestBuildSchemeRing:
    ...
    def test_ring_cbh_rcts_r0(self):
        smiles = 'O=C1CC(F)CN1'
        cbh = CBH.buildCBH(smiles)
        assert cbh.cbh_rcts[0] == {'[H][H]': 8.0}

    def test_ring_cbh_rcts_r1(self):
        smiles = 'O=C1CC(F)CN1'
        cbh = CBH.buildCBH(smiles)
        assert cbh.cbh_rcts[1] == {'N': 1, 'C': 6}
    
    def test_ring_cbh_rcts_r2(self):
        smiles = 'O=C1CC(F)CN1'
        cbh = CBH.buildCBH(smiles)
        assert cbh.cbh_rcts[2] == {'CN': 2, 'CC': 3}

    def test_ring_cbh_rcts_r3(self):
        smiles = 'O=C1CC(F)CN1'
        cbh = CBH.buildCBH(smiles)
        assert cbh.cbh_rcts[3] == {'CNC': 1, 'CC(N)=O': 1, 'CC(C)F': 1, 'CCC': 1, 'CCN': 1}
    
    def test_ring_cbh_pdts_r0(self):
        smiles = 'O=C1CC(F)CN1'
        cbh = CBH.buildCBH(smiles)
        assert cbh.cbh_pdts[0] == {'O': 1, 'F': 1, 'N': 1, 'C': 4}

    def test_ring_cbh_pdts_r1(self):
        smiles = 'O=C1CC(F)CN1'
        cbh = CBH.buildCBH(smiles)
        assert cbh.cbh_pdts[1] == {'CN': 2, 'CC': 3, 'C=O': 1, 'CF': 1}

    def test_ring_cbh_pdts_r2(self):
        smiles = 'O=C1CC(F)CN1'
        cbh = CBH.buildCBH(smiles)
        assert cbh.cbh_pdts[2] == {'CNC': 1, 'CC(N)=O': 1, 'CC(C)F': 1, 'CCC': 1, 'CCN': 1}

    def test_ring_cbh_pdts_r3(self):
        smiles = 'O=C1CC(F)CN1'
        cbh = CBH.buildCBH(smiles)
        assert cbh.cbh_pdts[3] == {'CC(F)CN': 1, 'CNC(C)=O': 1, 'CCNC': 1, 'CCC(C)F': 1, 'CCC(N)=O': 1}
    ...
    def test_ring_cbh_rcts_r0_sat(self):
        smiles = 'O=C1CC(F)CN1'
        cbh = CBH.buildCBH(smiles, 9)
        assert cbh.cbh_rcts[0] == {'[H][H]': 2.5, 'FF': 10.5}

    def test_ring_cbh_rcts_r1_sat(self):
        smiles = 'O=C1CC(F)CN1'
        cbh = CBH.buildCBH(smiles, 9)
        assert cbh.cbh_rcts[1] == {'N': 1, 'FC(F)(F)F': 10}
    
    def test_ring_cbh_rcts_r2_sat(self):
        smiles = 'O=C1CC(F)CN1'
        cbh = CBH.buildCBH(smiles, 9)
        assert cbh.cbh_rcts[2] == {'FC(F)(F)C(F)(F)F': 3, 'NC(F)(F)F': 2}

    def test_ring_cbh_rcts_r3_sat(self):
        smiles = 'O=C1CC(F)CN1'
        cbh = CBH.buildCBH(smiles, 9)
        assert cbh.cbh_rcts[3] == {'NC(=O)C(F)(F)F': 1, 'FC(F)(F)CC(F)(F)F': 1, 'NCC(F)(F)F': 1, 'FC(C(F)(F)F)C(F)(F)F': 1, 'FC(F)(F)NC(F)(F)F': 1}
    
    def test_ring_cbh_pdts_r0_sat(self):
        smiles = 'O=C1CC(F)CN1'
        cbh = CBH.buildCBH(smiles, 9)
        assert cbh.cbh_pdts[0] == {'O': 1, 'F': 6, 'N': 1, 'FC(F)(F)F': 4}

    def test_ring_cbh_pdts_r1_sat(self):
        smiles = 'O=C1CC(F)CN1'
        cbh = CBH.buildCBH(smiles, 9)
        assert cbh.cbh_pdts[1] == {'FC(F)(F)C(F)(F)F': 3, 'O=C(F)F': 1, 'FC(F)F': 5, 'NC(F)(F)F': 2}

    def test_ring_cbh_pdts_r2_sat(self):
        smiles = 'O=C1CC(F)CN1'
        cbh = CBH.buildCBH(smiles, 9)
        assert cbh.cbh_pdts[2] == {'NC(=O)C(F)(F)F': 1, 'FC(F)(F)CC(F)(F)F': 1, 'NCC(F)(F)F': 1, 'FC(C(F)(F)F)C(F)(F)F': 1, 'FC(F)(F)NC(F)(F)F': 1}
...
class TestBuildSchemeRadical:
    ...
    def test_rad_cbh_rcts_r1(self):
        smiles = '[C]C[C]C(F)(F)[CH2]'
        cbh = CBH.buildCBH(smiles)
        assert cbh.cbh_rcts[1] == {'[CH2]': 1, 'C': 4}
    
    def test_rad_cbh_rcts_r2(self):
        smiles = '[C]C[C]C(F)(F)[CH2]'
        cbh = CBH.buildCBH(smiles)
        assert cbh.cbh_rcts[2] == {'[CH]C': 2}

    def test_rad_cbh_rcts_r3(self):
        smiles = '[C]C[C]C(F)(F)[CH2]'
        cbh = CBH.buildCBH(smiles)
        assert cbh.cbh_rcts[3] == {'C[C]C': 1}
    
    def test_rad_cbh_pdts_r0(self):
        smiles = '[C]C[C]C(F)(F)[CH2]'
        cbh = CBH.buildCBH(smiles)
        assert cbh.cbh_pdts[0] == {'F': 2, '[CH2]': 1, '[CH]': 1, '[CH3]': 1, 'C': 2}

    def test_rad_cbh_pdts_r1(self):
        smiles = '[C]C[C]C(F)(F)[CH2]'
        cbh = CBH.buildCBH(smiles)
        assert cbh.cbh_pdts[1] == {'[CH]C': 2, '[CH2]C': 1, '[C]C': 1, 'CF': 2}

    def test_rad_cbh_pdts_r2(self):
        smiles = '[C]C[C]C(F)(F)[CH2]'
        cbh = CBH.buildCBH(smiles)
        assert cbh.cbh_pdts[2] == {'[CH]C([CH2])(F)F': 1, '[C]C[CH]': 1, 'C[C]C': 1}

    def test_rad_cbh_pdts_r3(self):
        smiles = '[C]C[C]C(F)(F)[CH2]'
        cbh = CBH.buildCBH(smiles)
        assert cbh.cbh_pdts[3] == {'[CH2]C(F)(F)[C]C': 1, '[C]C[C]C': 1}
    ...
    def test_rad_cbh_rcts_r0_sat(self):
        smiles = '[C]C[C]C(F)(F)[CH2]'
        cbh = CBH.buildCBH(smiles, 9)
        assert cbh.cbh_rcts[0] == {'FF': 8.0}

    def test_rad_cbh_rcts_r1_sat(self):
        smiles = '[C]C[C]C(F)(F)[CH2]'
        cbh = CBH.buildCBH(smiles, 9)
        assert cbh.cbh_rcts[1] == {'F[C]F': 1, 'F[C](F)F': 2, 'FC(F)(F)F': 4}
    
    def test_rad_cbh_rcts_r2_sat(self):
        smiles = '[C]C[C]C(F)(F)[CH2]'
        cbh = CBH.buildCBH(smiles, 9)
        assert cbh.cbh_rcts[2] == {'F[C]C(F)(F)F': 2, 'F[C](F)C(F)(F)F': 1}

    def test_rad_cbh_rcts_r3_sat(self):
        smiles = '[C]C[C]C(F)(F)[CH2]'
        cbh = CBH.buildCBH(smiles, 9)
        assert cbh.cbh_rcts[3] == {'FC(F)(F)[C]C(F)(F)F': 1, 'F[C]C(F)(F)[C](F)F': 1}
    
    def test_rad_cbh_pdts_r0_sat(self):
        smiles = '[C]C[C]C(F)(F)[CH2]'
        cbh = CBH.buildCBH(smiles, 9)
        assert cbh.cbh_pdts[0] == {'F[C]F': 1, 'F[C](F)F': 1, 'F': 4, 'FC(F)(F)F': 2, '[C]F': 1}

    def test_rad_cbh_pdts_r1_sat(self):
        smiles = '[C]C[C]C(F)(F)[CH2]'
        cbh = CBH.buildCBH(smiles, 9)
        assert cbh.cbh_pdts[1] == {'F[C]C(F)(F)F': 2, 'F[CH]F': 2, '[C]C(F)(F)F': 1, 'FC(F)F': 2, 'F[C](F)C(F)(F)F': 1}

    def test_rad_cbh_pdts_r2_sat(self):
        smiles = '[C]C[C]C(F)(F)[CH2]'
        cbh = CBH.buildCBH(smiles, 9)
        assert cbh.cbh_pdts[2] == {'FC(F)(F)[C]C(F)(F)F': 1, 'F[C]C(F)(F)[C](F)F': 1, '[CH2]C(F)(F)F': 1, '[C]C[C]F': 1}
...
class TestBuildSchemeBranch:
    ...
    def test_branch_cbh_rcts_r0(self):
        smiles = 'CC(CF)C(F)(F)Cl'
        cbh = CBH.buildCBH(smiles)
        assert cbh.cbh_rcts[0] == {'[H][H]': 7.0}

    def test_branch_cbh_rcts_r1(self):
        smiles = 'CC(CF)C(F)(F)Cl'
        cbh = CBH.buildCBH(smiles)
        assert cbh.cbh_rcts[1] == {'C': 6}
    
    def test_branch_cbh_rcts_r2(self):
        smiles = 'CC(CF)C(F)(F)Cl'
        cbh = CBH.buildCBH(smiles)
        assert cbh.cbh_rcts[2] == {'CC': 2}

    def test_branch_cbh_rcts_r3(self):
        smiles = 'CC(CF)C(F)(F)Cl'
        cbh = CBH.buildCBH(smiles)
        assert cbh.cbh_rcts[3] == {'CC(C)C': 1}
    
    def test_branch_cbh_pdts_r0(self):
        smiles = 'CC(CF)C(F)(F)Cl'
        cbh = CBH.buildCBH(smiles)
        assert cbh.cbh_pdts[0] == {'Cl': 1, 'F': 3, 'C': 4}

    def test_branch_cbh_pdts_r1(self):
        smiles = 'CC(CF)C(F)(F)Cl'
        cbh = CBH.buildCBH(smiles)
        assert cbh.cbh_pdts[1] == {'CCl': 1, 'CC': 3, 'CF': 3}

    def test_branch_cbh_pdts_r2(self):
        smiles = 'CC(CF)C(F)(F)Cl'
        cbh = CBH.buildCBH(smiles)
        assert cbh.cbh_pdts[2] == {'CC(C)C': 1, 'CCF': 1, 'CC(F)(F)Cl': 1}

    def test_branch_cbh_pdts_r3(self):
        smiles = 'CC(CF)C(F)(F)Cl'
        cbh = CBH.buildCBH(smiles)
        assert cbh.cbh_pdts[3] == {'CC(C)C(F)(F)Cl': 1, 'CC(C)CF': 1}
    ...
    def test_branch_cbh_rcts_r0_sat(self):
        smiles = 'CC(CF)C(F)(F)Cl'
        cbh = CBH.buildCBH(smiles, 9)
        assert cbh.cbh_rcts[0] == {'[H][H]': 0.5, 'FF': 9.5}

    def test_branch_cbh_rcts_r1_sat(self):
        smiles = 'CC(CF)C(F)(F)Cl'
        cbh = CBH.buildCBH(smiles, 9)
        assert cbh.cbh_rcts[1] == {'FC(F)(F)F': 9}
    
    def test_branch_cbh_rcts_r2_sat(self):
        smiles = 'CC(CF)C(F)(F)Cl'
        cbh = CBH.buildCBH(smiles, 9)
        assert cbh.cbh_rcts[2] == {'FC(F)(F)C(F)(F)F': 3}
    
    def test_branch_cbh_pdts_r0_sat(self):
        smiles = 'CC(CF)C(F)(F)Cl'
        cbh = CBH.buildCBH(smiles, 9)
        assert cbh.cbh_pdts[0] == {'Cl': 1, 'F': 6, 'FC(F)(F)F': 4}

    def test_branch_cbh_pdts_r1_sat(self):
        smiles = 'CC(CF)C(F)(F)Cl'
        cbh = CBH.buildCBH(smiles, 9)
        assert cbh.cbh_pdts[1] == {'FC(F)(F)C(F)(F)F': 3, 'FC(F)(F)Cl': 1, 'FC(F)F': 6}
...
class TestBuildSchemeIgnoreF2:
    ...
    def test_igf2_true_cbh_rcts_r0_sat(self):
        smiles = 'CC(=O)C(F)O'
        cbh = CBH.buildCBH(smiles, 9)
        assert cbh.cbh_rcts[0] == {'[H][H]': 5.0}

    def test_igf2_true_cbh_rcts_r1_sat(self):
        smiles = 'CC(=O)C(F)O'
        cbh = CBH.buildCBH(smiles, 9)
        assert cbh.cbh_rcts[1] == {'FC(F)(F)F': 7}
    
    def test_igf2_true_cbh_rcts_r2_sat(self):
        smiles = 'CC(=O)C(F)O'
        cbh = CBH.buildCBH(smiles, 9)
        assert cbh.cbh_rcts[2] == {'FC(F)(F)C(F)(F)F': 2}
    
    def test_igf2_true_cbh_pdts_r0_sat(self):
        smiles = 'CC(=O)C(F)O'
        cbh = CBH.buildCBH(smiles, 9)
        assert cbh.cbh_pdts[0] == {'C': 2.75, 'O': 2, 'FC(F)(F)F': 0.25}

    def test_igf2_true_cbh_pdts_r1_sat(self):
        smiles = 'CC(=O)C(F)O'
        cbh = CBH.buildCBH(smiles, 9)
        assert cbh.cbh_pdts[1] == {'FC(F)(F)C(F)(F)F': 2, 'O=C(F)F': 1, 'OC(F)(F)F': 1, 'FC(F)F': 4}
    ...
    def test_igf2_false_highest_rung_sat(self):
        smiles = 'CC(=O)C(F)O'
        cbh = CBH.buildCBH(smiles, 9, ignore_F2=False)
        assert cbh.highest_cbh == 2
    
    def test_igf2_false_cbh_rcts_r0_sat(self):
        smiles = 'CC(=O)C(F)O'
        cbh = CBH.buildCBH(smiles, 9, ignore_F2=False)
        assert cbh.cbh_rcts[0] == {'[H][H]': 2.0, 'FF': 8.0}
    ...
    def test_igf2_false_cbh_pdts_r0_sat(self):
        smiles = 'CC(=O)C(F)O'
        cbh = CBH.buildCBH(smiles, 9, ignore_F2=False)
        assert cbh.cbh_pdts[0] == {'O': 2, 'F': 5, 'FC(F)(F)F': 3}
