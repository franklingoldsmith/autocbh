...
class TestHf:
    ...
    def test_Hrxn_Hf_generic_sum_Hrxn(self):
        # OC[Pt]
        Hrxn_true = stoichiometric_math(energies_df=self.c.energies, full_rxn=self.cbh1_rxn, species=self.species,
                                        columns=self.methods_keys, convert_from_hartree_to_kjmol=True)
        Hf_partial_calc = stoichiometric_math(energies_df=self.c.energies, full_rxn=self.cbh1_rxn, species=self.species,
    ...
    def test_custom_Hrxn_fcn(self):
        ...
        self.c.energies.loc[CanonSmiles('CO.[Pt]'), 'beef_vdw_zpe'] = -910.330881 # reset since previous function sets it to nan

        def custom_hrxn_fcn(delE:dict, *args:str) -> float:
            """
            ...
            :**args:    [str] Dictionary keys to sum for single point energy. Typically the single point energy and zpe.
            """
        ...
        Hrxn_true = stoichiometric_math(energies_df=self.c.energies, full_rxn=self.cbh1_rxn, species=self.species,
                                        columns=['beef_vdw_E0', 'beef_vdw_zpe'], convert_from_hartree_to_kjmol=True)
        Hrxn_true_custom = 1/500*stoichiometric_math(energies_df=self.c.energies, full_rxn=self.cbh1_rxn, species=self.species,
                                        columns=['custom1', 'custom2'], convert_from_hartree_to_kjmol=True)
        Hf_partial_calc = stoichiometric_math(energies_df=self.c.energies, full_rxn=self.cbh1_rxn, species=self.species,
                                              columns='DfH', convert_from_hartree_to_kjmol=False)
        Hf_true = [np.round(v - Hf_partial_calc, 8) for v in [Hrxn_true, Hf_partial_calc]]
        Hf_true_custom = [np.round(v - Hf_partial_calc, 8) for v in [Hrxn_true_custom, Hf_partial_calc]]

        Hrxn, Hf = self.c.Hf(s=CanonSmiles('OC[Pt]'), rxn=self.cbh1_rxn, skip_precursor_check=True, hrxn_fcns={'custom':custom_hrxn_fcn})
        Hrxn = {k:np.round(v,8) for k, v in Hrxn.items()}
        Hf = {k:np.round(v,8) for k, v in Hf.items()}
        Hrxn_correct_dict = {'beef_vdw':np.round(Hrxn_true,8), 'ref':np.round(Hf_partial_calc,8), 'custom':np.round(Hrxn_true_custom, 8)}
